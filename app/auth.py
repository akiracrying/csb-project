...
def generate_jwt(user):
    # --- VULN: FLAW 5 - Security Misconfiguration, JWT expires in 365 days ---
    payload = {
        'user_id': user.id,
    ...
    # --- END VULN: FLAW 5 (JWT expiration) ---

    # --- FIX: FLAW 5 - Security Misconfiguration, short JWT expiration ---
    # payload = {
    #     'user_id': user.id,
    #     'username': user.username,
    #     'role': user.role,
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # --- FIX: FLAW 5 - Security Misconfiguration, check role from DB ---
